...
<?php
  ...
  if(isset($_POST["do_login"])) {
    $user = R::findOne('users', 'auth = ?', array($_POST['token']));
    if($user) {
      $_SESSION['logged_user'] = $_POST['token'];
      echo "<script>document.location.href = '/app.php';</script>";
    }
    else {
      echo '
...
<div class="wrapper">
  <div class="header"></div>
    <h3 class="sign-in sign-in__heading">Вход</h3>
   <div class="clear"></div> 
  <form action="index.php" method="post">
      <div>
        <label class="lock" for="password" style="margin-top:5px;margin-left:-2px;">
        ...
        <input type="password" name="token" id="name" placeholder="Ключ" />
      </div> 
     <div>
